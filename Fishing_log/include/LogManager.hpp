...
namespace hk {
     ...
     * Create,Read,Uptade,Delete for fishing log
     * Calculations and history display
     */
    class LogManager {
    private:
        /**
         * @brief The main Json object storing all log data
         */
        nlohmann::json m_jsonData;
        /**
         * @brief File path to the Json file
         */
        std::string m_jsonPath;

    public:
        /**
         * @brief Construct the LogManager and loads existing data
         * 
        ...
        /**
         * @brief Loads data form the Json file into memory
         * 
         * Tries to open and parse the file at m_jsonPath
         * If the file doesnt exits, is empty, or contians invalid json
         * it initlializes a new json structure
         */
        void load() {
        ...
        /**
         * @brief Saves the current memory state to the Json file
         * 
         * Writes the m_jsonData object to m_jsonPath
         */
         ...
         * @return T The validated value entered by the user
         */
        ...
        /**
         * @brief Validates a string input against a specific type
         * 
         * Attemps to convert a string into type T.
         * Sets a success flag
         * 
         * @tparam T the target type
         * @param input The string input to check
         * @param accept Reference to a boolean falg
         * @return T The converted value if successful
         */
        ...
        /**
         * @brief Interface to create a new daily log
         * 
         * Creates a new Log entry and initializes an empty fish array for that day
        ...
        void createNewLog() {
            ...
            while (true) {
                ...
                ///Check the inputed data format

                start_hour = checkInput<int>("Enter start hour (only hour 0-23): ");
                ...
                ///Gets form user starting hour and returns it in int value, prevents from errors
                ...
                ///Creating fish array in log object
        ...
        /**
         * @brief Interface to edit an existing log entry
         * 
         * Provides a submenu
         * 
         * Display the last 5 log dates for quick selection
         */
        ...
        void editLog() {
            ...
            for (const auto &[dateKey, dateLog]: m_jsonData["log"].items()) {
                dates.push_back(dateLog.value("date", "unknown"));
            }

            for (int i = dates.size() - 1; i >= 0 && count < 5; i--) {
                ...
                switch (selectEdit) {
                    case 1: {
                        ...
                        while (true) {
                            ...
                            std::cout << "Current weather is: " << m_jsonData["log"][date]["weather"] << std::endl
                                      << std::endl;
                            ...
                            ///Display current Log info, and getting new data set
                    ...
                    case 2: {
                        ...
                        std::string bait;
                        std::string gb;
                        std::string fishListPath = "./fish_list/fishList.json";

                        hk::FishTypeProvider::initialize(fishListPath);
                        ...
                        while (true) {
                            ...
                            if (!(checkName(name))) {
                                std::cout << "There is no fish: " << name
                                          << " in the fishList file, please press enter to continue";
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                continue;
                            } else {
                            ...
                            weight = checkInput<double>("Enter weight [kg]: ");
                            if(weight <= 0 ){
                                std::cout << "Invalid weight value,press enter to continue" << std::endl;
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                std::cin.get();
                                continue;
                            }
                            length = checkInput<double>("Enter length [cm]: ");
                             if(length <= 0 ){
                                std::cout << "Invalid length value,press enter to continue" << std::endl;
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                std::cin.get();
                                continue;
                            }
                            hour = checkInput<int>("Enter hour [only hour]: ");

                            if (hour <= 0 || hour > 23) {
                                std::cout << "Invalid hour, please press enter to contiune" << std::endl;
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                std::cin.get();
                                continue;
                            }
                            ...
                            if (hk::FishTypeProvider::getFishType(name) == "Predator") {
                                std::cout << "Enter lure: ";
                    ...
                    case 3: {
                        ...
                        if (fishArray.empty()) {
                            ...
                            for (const auto &fish: fishArray) {
                                std::cout << "ID: " << fish.value("id", 0) << " | species: "
                                          << fish.value("name", "unknown") << ", length: " << fish.value("length", 0.0)
                                          << " cm " << " ,weight: " << fish.value("weight", 0.0);
                                std::cout << std::endl;
                            }
                        }

                        ///List of all fishes catched at searched day
                        ...
                        while (true) {
                            ...
                            std::cout << std::endl;
                            std::cout << "You cant edit: id and name. To do this please delete fish and create new "
                                      << std::endl;
                            std::cout << "To change parameter type parameter,new_value: " << std::endl;
                        ...
                        ///Set new values for fish based on id
                    ...
                    case 4: {
                        ...
                        while (true) {
                            ...
                            if (fishArray.empty()) {
                                std::cout << "There arent fishes for this day,to continue press enter" << std::endl;
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                std::cin.get();
                            } else {
                                for (const auto &fish: fishArray) {
                                    std::cout << "ID: " << fish.value("id", 0) << " | species: "
                                              << fish.value("name", "unknown") << ", length: "
                                              << fish.value("length", 0.0) << " cm " << " ,weight: "
                                              << fish.value("weight", 0.0);
                                    std::cout << std::endl;
                                }
                            }
                            ...
                            if (ans == 'y' || ans == 'Y') {
                                ...
                                std::cout << "Fish deleted,to contiune press enter" << std::endl;
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                std::cin.get();
                                continue;
                            } else {
                                std::cout << "Aborted, to continue press enter" << std::endl;
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                std::cin.get();
                                continue;
                            }
                    ...
                    case 5: {
                        ...
                        while (true) {
                            ...
                            if (m_jsonData["log"].contains(newDate)) {
                                std::cout << "Log already exist, please delete or edit it, press enter to continue"
                                          << std::endl;
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                std::cin.get();
                                continue;
                                ...
                                save();
                                std::cout << "Succesfuly changed date from: " << date << " to new date: " << newDate
                                          << std::endl;
                                std::cout << "Press enter to continue" << std::endl;
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                                std::cin.get();
        ...
        /**
         * @brief Deletes an entire log entry for a specific date
         * 
         * Display last 5 logs and pormpts the user for a date to remove
        ...
        /**
         * @brief Calculates and displays personal records.
         * 
         * Scans all logs to find
         * 
         * Displays and results along withe the date and lure,bait,groundbait used
         */
        void recordsLog() {

            ///View records in two categories Predators and Coarse Fishes. The longest one and the heaviest one
            ...
            if (longestPredator != nullptr) {
                std::cout << "Longest: " << longestPredator->at("name").get<std::string>() << " "
                          << longestPredator->at("length").get<double>() << " cm " << ",lure: "
                          << longestPredator->at("lure").get<std::string>();
                std::cout << " ,date: " << predatorLongestDate << std::endl;
            }

            if (heaviestPredator != nullptr) {
                std::cout << "Heaviest: " << heaviestPredator->at("name").get<std::string>() << " "
                          << heaviestPredator->at("weight").get<double>() << " kg " << ",lure: "
                          << heaviestPredator->at("lure").get<std::string>();
            ...
            if (longestCoarse != nullptr) {
                std::cout << "Longest: " << longestCoarse->at("name").get<std::string>() << " "
                          << longestCoarse->at("length").get<double>() << " cm " << ",bait: "
                          << longestCoarse->at("bait").get<std::string>();
                std::cout << " ,ground bait: " << longestCoarse->at("ground_bait").get<std::string>() << " ,date: "
                          << coarseLongestDate << std::endl;
            }

            if (heaviestCoarse != nullptr) {
                std::cout << "Heaviest: " << heaviestCoarse->at("name").get<std::string>() << " "
                          << heaviestCoarse->at("weight").get<double>() << " kg " << ",bait: "
                          << heaviestCoarse->at("bait").get<std::string>();
                std::cout << " ,ground bait: " << heaviestCoarse->at("ground_bait").get<std::string>() << " ,date: "
                          << coarseHeaviestDate << std::endl;
            } else {
                std::cout << "No record found." << std::endl;
            }
        }
        ...
        /**
         * @brief Displays the full history of fishing log
         * 
         * Printing all logs
         */
        ...
        void historyLog() {
            ...
            for (const auto &[dateKey, dateLog]: m_jsonData["log"].items()) {
                ...
                std::cout << std::string(wIndex + wName + wHour + wLen + wWeight + wLure + wGround, '-') << std::endl;
                ...
                for (const auto &fish: dateLog["fishes"]) {
                    ...
                    if (fish.value("type", "unknown") == "Coarse_Fish") {
                        ...
                        std::cout << std::left
                                  ...
                                  << std::setw(wHour) << fish.value("hour", 0)
                    ...
                    ///Fishes like Predators dont have groundbait so writed " - "
